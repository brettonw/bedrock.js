let Forms = function () {
    ...
    _.init = function (parameters) {
        ...
        for (let input of parameters.inputs) {
            ...
            switch (input.type) {
                case "text": {
                     ...
                     */
                    inputElement = addElement (formDivElement, "input", { id: (formName + INPUT + input.name), type: "text", class: "form-input", placeholder: input.placeholder });
                    break;
                }
                case "checkbox": {
                    /*
                     ...
                     */
                    inputElement = addElement (formDivElement, "input", { id: (formName + INPUT + input.name), type: "checkbox", class: "form-input", checked: input.checked });
                    break;
                }
                case "select": {
                     ...
                     */
                    inputElement = addElement (formDivElement, "select", { id: (formName + INPUT + input.name), class: "form-input" });
                    for (let option of input.options) {
                        let optionElement = addElement (inputElement, "option", { value: option.value, label: option.label });
            ...
            // and now add the error
            let errorElement = addElement (formDivElement, "div", { id: (formName + ERROR + input.name), class: "form-error", innerHTML: "REQUIRED" });

            // and fill the inputs object
            inputs[input.name] = {
                name: input.name,
                type: input.type,
        ...
        addElement (formDivElement, "input", { type: "button", value: "SUBMIT", class: "bedrockClearButton" });
